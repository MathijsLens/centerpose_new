...
def add_size():
    final_json=[]
    data_root="data/outf_all/ford/"
    with open(data_root+"order_val.json", 'r') as f:
        data=json.load(f)
    print(f"length before processing: {len(data)}")
    
    # open  csv for parcelID, dimentions
    df_id=pd.read_csv("csv/csv.csv")
    # print(df_id.head(1))
    
    df_dim=pd.read_csv("csv/dimentions.csv")
    # print(df_dim.head(1))
    for num, test_dict in enumerate(data):
        name=test_dict["image_name"]
        name="100-"+ name.split('_')[-1].split('.')[0]
        # print(name)
        index=df_id.index[df_id["Picture ID"] == name]
        parcel_id=df_id.iloc[index]["Parcel_ID"].values
        # print(df_id.iloc[index])
        # print(parcel_id)
        
        dim_index=df_dim.index[df_dim["ParcelID"]==parcel_id[0]].values
        l = df_dim.iloc[dim_index[0]]["ParcelLength"]
        w = df_dim.iloc[dim_index[0]]["ParcelWidth"]
        d = df_dim.iloc[dim_index[0]]["ParcelDepth"]
        size=[int(l),int(w),int(d)]
        
        final_json.append({"image_name": test_dict["image_name"], 
                            "points": test_dict["points"],
                            "size": size})
        print(num)
    print(f"length after processing: {len(final_json)}")
    with open(data_root+"order_val_size.json", 'w') as f:
...
def main(verbose=False):
    final_json=[]
    data_root="data/outf_all/ford/"
    with open(data_root+"all.json", 'r') as f:
        data=json.load(f)
    print(f"length before processing: {len(data)}")
    ...
    for num, test_dict in enumerate(data):
        ...
        # debug purpes
        if verbose:
            m_img=plt.imread(data_root+"/all/"+test_dict["image_name"])
    ...
    print(f"length after processing: {len(final_json)}")
    with open(data_root+"order_train.json", 'w') as f:
        json.dump(final_json, f)
